...
class DPR_Net(nn.Module):
    ...
    def forward(self, img_distorted, 
                text_labels=None, 
                # [수정 1] dpr_modules.py와 변수명 통일 (text_mask -> text_attention_mask)
        ...
        if mode == 'train':
            # 3. 훈련 모드: L_text, hidden_state, logits 반환
            loss_text, hidden_state, logits = self.llm_module(
                vision_tokens, 
                text_labels=text_labels, 
                # [수정 2] dpr_modules.py와 변수명 통일
                text_attention_mask=text_attention_mask
            )
            
            # 4. FiLM 신호 생성
            # (hidden_state는 [B, 257+SeqLen, D]이므로, 시각 토큰 부분만 잘라냄)
            vision_hidden_state = hidden_state[:, :vision_tokens.shape[1], :]
            film_signals = self.film_head(vision_hidden_state) 
            cls_hidden_state = vision_hidden_state[:, 0:1, :] # L_consistency용 [B, 1, D]
            ...
            return {
                ...
                'logits': logits, # (L_consistency 계산용)
            }

        else: # (mode == 'eval')
            # 3. 추론 모드: 텍스트 ID와 hidden_state 반환
            generated_ids, hidden_state, _ = self.llm_module(vision_tokens) 
            
            # 4. FiLM 신호 생성
            # (hidden_state는 [B, 257, D]이므로, 시각 토큰 부분만 사용)
